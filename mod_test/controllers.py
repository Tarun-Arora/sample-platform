...
def get_data_for_test(test, title=None) -> Dict[str, Any]:
    ...
    if len(test.progress) == 0:
        ...
        gcp_instance_test = g.db.query(TestProgress.test_id, label('time', func.group_concat(TestProgress.timestamp))).
        filter(TestProgress.test_id.in_(queued_gcp_instance_entries)).group_by(TestProgress.test_id).all()
